...
const expected={'quant-returns-01':40,'quant-returns-02':40,'quant-benchmarking-03':45,'quant-tvm-04':50,'quant-statistics-05':45,'quant-probability-06':50,'quant-distributions-07':50,'quant-sampling-08':45,'quant-hypothesis-09':45,'quant-regression-10':50,'quant-simulation-11':45,'quant-data-ai-12':50,'economics-firm-market-01':60};
for(const[id,count]of Object.entries(expected)){const qs=questions.filter(q=>q.lessonId===id);if(qs.length!==count)failures.push(`${id}: expected ${count} questions`);if(cards.filter(c=>c.lessonId===id).length!==count)failures.push(`${id}: expected ${count} flashcards`);if(['quant-statistics-05','quant-probability-06','quant-distributions-07','quant-sampling-08','quant-hypothesis-09','quant-regression-10','quant-simulation-11','quant-data-ai-12','economics-firm-market-01'].includes(id)){const positions=[0,1,2].map(index=>qs.filter(q=>q.correctChoiceIndex===index).length);if(positions.some(n=>n<Math.floor(count*.2)))failures.push(`${id}: answer positions are materially imbalanced`)}}
if(questions.filter(q=>q.lessonId==='economics-firm-market-01'&&q.supplementary).length!==15||cards.filter(c=>c.lessonId==='economics-firm-market-01'&&c.supplementary).length!==15)failures.push('economics-firm-market-01: supplementary split must be 15 questions and 15 cards');
...
for(const id of ['quant-statistics-05','quant-probability-06','quant-distributions-07','quant-sampling-08','quant-hypothesis-09','quant-regression-10','quant-simulation-11','quant-data-ai-12','economics-firm-market-01'])if(!registry.includes(id))failures.push(`${id}: missing interactive registration`);
