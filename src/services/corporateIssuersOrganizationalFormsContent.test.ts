...
describe('Corporate Issuers LM1 organizational forms and ownership',()=>{
 ...
 it('preserves LM1 while LM2 is published and LM3 through LM7 remain pending',()=>{expect(officialCorporateIssuersModules).toHaveLength(7);expect(verifiedOfficialCorporateIssuersModules).toHaveLength(2);expect(officialCorporateIssuersModules.filter(module=>module.status==='pending')).toHaveLength(5)});
});
