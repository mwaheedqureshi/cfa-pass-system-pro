...
describe('Corporate Issuers LM1 organizational forms and ownership',()=>{
 ...
 it('preserves LM1 while LM2 through LM4 are published and LM5 through LM7 remain pending',()=>{expect(officialCorporateIssuersModules).toHaveLength(7);expect(verifiedOfficialCorporateIssuersModules).toHaveLength(4);expect(officialCorporateIssuersModules.filter(module=>module.status==='pending')).toHaveLength(3)});
});
