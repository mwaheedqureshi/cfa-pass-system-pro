...
describe('Corporate Issuers LM1 organizational forms and ownership',()=>{
 ...
 it('publishes only LM1 and leaves LM2 through LM7 pending',()=>{expect(officialCorporateIssuersModules).toHaveLength(7);expect(verifiedOfficialCorporateIssuersModules).toHaveLength(1);expect(officialCorporateIssuersModules.filter(module=>module.status==='pending')).toHaveLength(6)});
});
