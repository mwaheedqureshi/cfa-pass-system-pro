...
describe('Corporate Issuers LM1 organizational forms and ownership',()=>{
 ...
 it('preserves LM1 while LM2 through LM7 are published',()=>{expect(officialCorporateIssuersModules).toHaveLength(7);expect(verifiedOfficialCorporateIssuersModules).toHaveLength(7);expect(officialCorporateIssuersModules.filter(module=>module.status==='pending')).toHaveLength(0)});
});
