...
describe('Corporate Issuers LM1 organizational forms and ownership',()=>{
 ...
 it('preserves LM1 while LM2 through LM6 are published and LM7 remains pending',()=>{expect(officialCorporateIssuersModules).toHaveLength(7);expect(verifiedOfficialCorporateIssuersModules).toHaveLength(6);expect(officialCorporateIssuersModules.filter(module=>module.status==='pending')).toHaveLength(1)});
});
