...
describe('Corporate Issuers LM1 organizational forms and ownership',()=>{
 ...
 it('preserves LM1 while LM2 and LM3 are published and LM4 through LM7 remain pending',()=>{expect(officialCorporateIssuersModules).toHaveLength(7);expect(verifiedOfficialCorporateIssuersModules).toHaveLength(3);expect(officialCorporateIssuersModules.filter(module=>module.status==='pending')).toHaveLength(4)});
});
