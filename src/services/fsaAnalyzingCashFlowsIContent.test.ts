...
describe('FSA analyzing statements of cash flows I content',()=>{
 ...
 it('registers the stable lesson and remains verified as LM7 is added',()=>{const lesson=lessons.find(l=>l.id==='fsa-04-analyzing-statements-of-cash-flows-i');expect(lesson?.formulaIds).toHaveLength(8);expect(verifiedOfficialFsaModules).toHaveLength(8);expect(officialFsaModules.filter(m=>m.status==='pending')).toHaveLength(4)});
});
