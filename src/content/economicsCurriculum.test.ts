...
describe('2027 Economics curriculum map',()=>{
 ...
 it('marks exactly ECO-LM1 through ECO-LM4 as verified after this release',()=>{
  expect(verifiedOfficialEconomicsModules.map(m=>m.id)).toEqual(['ECO-LM1','ECO-LM2','ECO-LM3','ECO-LM4']);
 });
 ...
 it('derives economics lesson ids only from published modules',()=>{
  expect(economicsLessonIds).toEqual(['economics-firm-market-01','economics-02-understanding-business-cycles','economics-03-fiscal-policy','economics-04-monetary-policy']);
 });
});
